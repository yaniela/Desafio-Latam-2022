...
colors = ["#005cb9", "#3cb4e5","#00b189","#70d44b","#fed925",     "#ffa400" ,"#ff4713","#323e48"]
...
#Recibe el dataframe y devuelve la gráfica de densidad, con media y std, de cada columna del dataframe.
# Funciona ok menos con la variable OUT_M que tiene 0 varianza

def plot_multiple_kdeplot(df_dist, fig_largo, fig_alto):
    '''''
...
def multiple_lineplot_secundary_y_axis(df,  var_y,list,alto,largo):
 ...
 for i in range(0,len(list)):
   ...
   #axe[i].axhline(marker, color = 'r', ls = '--' )
...
def multiple_sns_lineplot_secundary_y_axis(df,var_x, var_y,list,alto,largo):
 ...
 ax.legend(loc="upper right", bbox_to_anchor=(1,1), bbox_transform=axe[i].transAxes)
...
def sns_pairplot(df, col_hue):
  ...
  sns.pairplot(df, hue=col_hue, palette="dark")
  return plt.show()
...
def bar_subplot_mosaic(data, dict, categorical_features, largo, alto): 
  '''''
    Devuelve varios barplots en mosaico con tamaño ajustado a la imagen.  
        
    Parametros:
    data: dataframe con los datos 
    alto: alto de la figura que contiene los subgráficas
    largo: largo de la figura que contiene los subgráficas
    dict: diccionario con las posiciones de cada subgráfica en el layout general. 
          Ej: [["A","B"],["C","C"]] devuelve las graficas A y B en la parte superior
           y la gráfica C en la parte inferior del tamaño de las graficas A y B
    categorical_features: columnas que se desean graficar
 '''''

  axes = plt.figure(constrained_layout=True,figsize=(largo, alto) ).subplot_mosaic(dict)

  for i, categorical_feature in enumerate(data[categorical_features]):
      data[categorical_feature].value_counts(sort=False).plot(kind="bar", ax=axes[categorical_features[i]]).set_title(categorical_feature)
